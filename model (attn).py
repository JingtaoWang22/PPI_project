...
class PPI_predictor(nn.Module):
    ...
    def forward(self, inputs): ## inputs=[p1,p2,interaction]
        ...
        p1=self.W_ff1(p1_vector)  # k x dim
        ...
        query = self.linears[0](p1).view(l1,-1,self.heads,d_k).transpose(1,2)
        key = self.linears[1](p2).view(l2,-1,self.heads,d_k).transpose(1,2)
        value1 = self.linears[2](p1).view(l1,-1,self.heads,d_k).transpose(1,2)
        value2 = self.linears[3](p2).view(l2,-1,self.heads,d_k).transpose(1,2)
